...
def handle_packet(pkt):
	...
	global allowed_ssh_connections
	...
	if IP in current_packet and TCP in current_packet:
		...
		if (current_packet['TCP'].flags == TCP_SYN):
		...
		elif dst_port == SSH_PORT and existing_connection(src_ip, dst_ip, src_port, dst_port): 
			if current_packet['TCP'].flags == TCP_ACK and src_ip not in allowed_ssh_connections:
				if not silent:
					send_reset(current_packet)
				print '[DROP] Attempt to establish SSH connection from', current_packet['IP'].src + '.'
				packet_buffer.pop(routing_tuple)
				pkt.drop()
			else:
				pkt.accept()

		elif (src_port in HTTP_PORTS or dst_port in HTTP_PORTS) and existing_connection(src_ip, dst_ip, src_port, dst_port):			
			payload = current_packet['TCP'].payload
			# if there are more fragments and not enough collected yet, add to buffer.
			if current_packet['IP'].flags == MF_FLAG and len(packet_buffer[routing_tuple]) <= MAX_FRAGMENT_CACHE:
			...
			# if too many fragments collected, accept and reset buffer for current connection.
			...
			if dst_port in HTTP_PORTS:
				...
				if len(header) != 0 and header[0] == 'GET':
					...
					pkt.accept()
			elif src_port in HTTP_PORTS and payload_so_far[0:15] == 'HTTP/1.1 200 OK':
		...
		if len(icmp.load) == 42:
			...
			if src_ip in allowed_ssh_connections.keys() and allowed_ssh_connections[src_ip] == current_packet['ICMP'].id:
...
# -------------------------------------------------------------------------------------------------------------------------------
	...
	#     payload = current_packet['TCP'].payload
	#     # if there are more fragments and not enough collected yet, add to buffer.
	#     if current_packet['IP'].flags == 1 and len(
	#             packet_buffer[routing_tuple]) <= MAX_FRAGMENT_CACHE:
	#         packet_buffer[routing_tuple][current_packet['TCP'].seq] = str(
	#                 payload)
	#         payload_so_far = assemble_payload_fragments(
	#                 packet_buffer[routing_tuple])
	#     # if too many fragments collected, accept and reset buffer for current connection.
	#     elif len(packet_buffer[routing_tuple]) > MAX_FRAGMENT_CACHE:
	#         packet_buffer.pop(routing_tuple)
	...
	#             if current_packet['TCP'].flags == TCP_SYN or current_packet['TCP'].flags == TCP_SYN_ACK:
	#                 pkt.accept()
	#             # catch the final ACK packet and block the SSH
	#             # connection by sending reset to both sides if
	#             # connection is not allowed
	#             elif current_packet['TCP'].flags == TCP_ACK and src_ip not in allowed_ssh_connections:
	#                 if not silent:
	#                     send_reset(current_packet)
	#                 print '[DROP] Attempt to establish SSH connection from', current_packet['IP'].src + '.'
	#                 packet_buffer.pop(routing_tuple)
	...
	#         if len(allowed_ssh_connections) >= MAX_CONNECTION_BUFFER_SIZE:
	#             allowed_ssh_connections.clear()
	#         allowed_ssh_connections.add(src_ip)
	#         pkt.accept()
	# else:
	#     pkt.accept()
...
# # infers a subnet address from IP
...
if __name__ == '__main__':
	...
	with open(conf_filename) as f:
		config_file = map(str.strip, f.readlines())
	...
	magic_numbers = filter(lambda mnum: mnum != '',
						   [cur.split('-')[1] for cur in config_file])
